...
def drop_last(iterable,n):
    i = iter(iterable)
    ele_list = [next(i) for _ in range(n)]
    try:
        while True:
            ele_list.append(next(i))
            yield ele_list[0]
            ele_list.pop(0)
...
def min_key_order(adict):
    ...
    try:
        while True:
            adict_copy = dict(adict)
            a = iter(sorted(adict_copy))
            b = next(a)
            if key_list:
                while b in key_list or b <= key_list[-1]:
                    b = next(a)
            else:
                while b in key_list:
                    b = next(a)
